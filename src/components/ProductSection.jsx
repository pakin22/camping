...
const ProductSection = () => {
    ...
    const startEdit = (product) => {
        ...
        product.variants?.forEach(v => {
            if (!colors.find(c => c.hex === v.colorHex)) colors.push({ name: v.color, hex: v.colorHex });
            if (!sizes.includes(v.size)) sizes.push(v.size);
        });
        setSelectedColors(colors); setSelectedSizes(sizes); setVariants(product.variants || []);
                ...
                <div style={styles.card}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <div style={{ marginBottom: '16px' }}>
                            ...
                            <input type="text" placeholder="เช่น เสื้อยืด Oversize" value={name} onChange={(e) => setName(e.target.value)} style={styles.input} required />
                        </div>
                        <div style={{ marginBottom: '16px' }}>
                                    ...
                                    <tr key={item.id} style={{ borderBottom: '1px solid #f1f5f9', transition: '0.2s', backgroundColor: isFlashActive ? '#fff1f0' : 'transparent' }}>
                                        ...
                                        <td style={{ padding: '15px' }} align="right">
                                            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                                                ...
                                                <button onClick={async () => { /* ... ฟังก์ชันลบเดิม ... */ }} style={{ border: 'none', background: '#fff1f0', color: '#ef4444', padding: '10px', borderRadius: '12px', cursor: 'pointer' }}><DeleteOutline fontSize="small"/></button>
                                            </div>
                                        </td>
                                    </tr>
                                );
